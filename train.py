...
def get_train_args():
    ...
    train_params.add_argument("--env", type=str, default='maze-random-10x10-plus-v0', help="Environment to use (must have RGB image state space and discrete action space)")
    train_params.add_argument("--render", type=bool, default=True, help="Whether or not to display the environment on the screen during training")
    train_params.add_argument("--random_seed", type=int, default=1234, help="Random seed for reproducability")
    train_params.add_argument("--frame_width", type=int, default=50, help="Frame width after resize.")
    train_params.add_argument("--frame_height", type=int, default=50, help="Frame height after resize.")
    train_params.add_argument("--frames_per_state", type=int, default=4, help="Sequence of frames which constitutes a single state.")

    # Training parameters
    train_params.add_argument("--num_steps_train", type=int, default=5000000, help="Number of steps to train for")    
    ...
    train_params.add_argument("--replay_mem_size", type=int, default=100000, help="Maximum size of replay memory buffer")
    ...
    train_params.add_argument("--epsilon_step_end", type=int, default=500000, help="After how many steps to stop decaying the exploration rate.")
    train_params.add_argument("--discount_rate", type=float, default=0.99, help="Discount rate (gamma) for future rewards.")
    train_params.add_argument("--update_target_step", type=float, default=1000, help="Copy current network parameters to target network every N steps.")
    train_params.add_argument("--save_ckpt_step", type=float, default=25000, help="Save checkpoint every N steps")
    train_params.add_argument("--save_log_step", type=int, default=100, help="Save logs every N steps")
...
def train(args):
    ...
    for random_step in range(1, args.initial_replay_mem_size+1):
        ...
        frame = preprocess_image(env.render(), args.frame_width, args.frame_height)
    ...
    for train_step in range(start_step+1, args.num_steps_train+1):      
        ...
        for _ in range(0, args.train_frequency):
            ...
            if terminal or ep_steps == args.max_ep_steps:  
                ...
                print('Episode final reward: ', episode_rewards)
